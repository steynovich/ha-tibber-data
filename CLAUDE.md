...
- 2025-11-11: Fixed critical entity recreation issue after restart - entities now always recreated from entity registry even when capabilities missing from API response, preventing "no longer provided" errors (v1.0.49 - complete fix, v1.0.48 was incomplete)
- 2025-11-11: Fixed entity availability logic after restart - entities now remain available even when capabilities missing from first coordinator update (v1.0.48 - incomplete fix, see v1.0.49)
- 2025-11-11: Removed temporary diagnostic WARNING logs - cleaned up log output by removing diagnostic logs added for troubleshooting, keeping only legitimate WARNING logs for actual error conditions
- 2025-11-11: Simplified connectivity sensor attributes - connectivity sensors (WiFi, Cellular) now only display last_seen in extra_state_attributes, removing all other connectivity-related attributes for cleaner UI
- 2025-11-11: Fixed entities showing as "no longer provided" after restart - changed `async_add_entities()` to use `update_before_add=False` to prevent entity creation from failing when initial coordinator data is incomplete or timing issues cause temporary unavailability, ensuring all entities are registered even if first update has issues. Added comprehensive error logging for entity creation failures to help diagnose issues.
- 2025-11-11: **Tibber API Breaking Change** - Tibber changed energy flow capability naming from `load.source.battery` to `battery.source.load` (reversed perspective). This affects energy flow entities. Old entities will show as "no longer provided" and must be manually deleted from Settings → Devices & Services → Entities. The integration correctly handles the new API format, so new entities are created with updated names (e.g., "Battery from Load" instead of "Load from Battery"). Update dashboards/automations to use new entity names.
- 2025-11-11: **COMPLETE FIX: Persistent Entity Availability** - Implemented capability history storage to definitively solve all entity availability issues (v1.0.51). Previous attempts (v1.0.48-v1.0.50) used entity registry but failed when users deleted entities. New solution: Persistent storage (`.storage/tibber_data_capability_history`) tracks ALL capabilities/attributes ever seen for each device, independent of entity registry. Benefits: Works even when entities are deleted, works when capabilities missing from API response, entities always recreated from persistent history. Storage automatically updated when new capabilities appear in API. Removed entity registry dependency entirely. This is the definitive solution that works in all scenarios including deleted entities and API timing issues.
